...
if  clave1 == "1963":
    ...
    # desde = '2000-01-01'
    desde = fecha_actual - timedelta(days=270)  # tomo dos año mio
    desde1 = '2000-01-01'
    ...
    if st.button("Ingresar"):
        ...
        data = ticker.history(start=desde1, end=hoy, auto_adjust=True)
        ...
        if not data.empty:
            ...
            ############################################################
            # máximo histórico
            vmax1 = data.Close.cummax()
            # mínimo histórico
            vmin1= data.Close.cummin()
            ...
            vmax=round(vmax1.iloc[-1],2)
            vmin=round(vmin1.iloc[-1],2)
            vclo=round(data.iloc[-1]["Close"],2)
            ...
            plt.plot(xdate, data.maxHist, label='Máximo: {}'.format(vmax))
            plt.plot(xdate, data.minHist, label='Mínimo: {} '.format(vmin))
            plt.plot(xdate, data.Close, label='Cierre: {} '.format(vclo))
            ...
            plt.plot(data.index, resultado_adx['+DI'],'-', lw=1, label='+DI', color='green')
            plt.plot(data.index, resultado_adx['-DI'],'-', lw=1, label='-DI', color='red')
            ...
            plt.axhline(50, linestyle='-', linewidth=1, color='blue')
            ...
            ax.plot(data.Close, '-k', lw=1, label='Precio')
            ax.plot(data.sma_fast, '--g', lw=1, label='SMA Rapida (14)')
            ax.plot(data.sma_slow, '--r', lw=1, label='SMA Lenta (100)')
            ...
            st.title("Indicadores para Fundamental")

            BASE_URL = "https://financialmodelingprep.com"
            path = f"/api/v3/ratios-ttm/{symbol}?apikey=c96830e4208288471b04b8f620beaf86"

            #tabla = requests.get('https://financialmodelingprep.com/api/v3/ratios-ttm/DIS?apikey=c96830e4208288471b04b8f620beaf86').json()
            tabla = requests.get(BASE_URL + path).json()
            datos = pd.Series(tabla)
            datos1 = datos[0]
            metricas_deseadas = ['peRatioTTM', 'priceCashFlowRatioTTM', 'priceToSalesRatioTTM', 'enterpriseValueMultipleTTM', 'priceToBookRatioTTM', 'debtEquityRatioTTM']
            for clave in metricas_deseadas:
                valor=datos1.get(clave)
                if valor is None:
                    continue
                else:
                    if clave == 'peRatioTTM':
                        umbral_bajo = 10.0
                        umbral_alto = 20.0
                    elif clave == 'priceCashFlowRatioTTM':
                        umbral_bajo = 5.0
                        umbral_alto = 15.0
                    elif clave == 'priceToSalesRatioTTM':
                        umbral_bajo = 1.0
                        umbral_alto = 3.0
                    elif clave == 'enterpriseValueMultipleTTM':
                        umbral_bajo = 10.0
                        umbral_alto = 20.0
                    elif clave == 'priceToBookRatioTTM':
                        umbral_bajo = 1.0
                        umbral_alto = 3.0
                    elif clave == 'debtEquityRatioTTM':
                        umbral_bajo = 0.5
                        umbral_alto = 1.0
                    if valor < umbral_bajo:
                        st.write(f"{clave}:  {round(valor,4)} . La acción podría estar subvalorada. Es un valor bajo.")
                    elif valor >= umbral_bajo and valor <= umbral_alto:
                        st.write(f"{clave}: {round(valor,4)} . Está en línea con el promedio de la industria o el mercado.")
                    else:
                        st.write(f"{clave}:  {round(valor,4)} . La acción podría estar sobrevalorada. Es un valor alto.")
            st.write(tabla)
